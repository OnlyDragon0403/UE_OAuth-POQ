...
void AHttpServer::StartServer()
{
	...
	if (httpRouter.IsValid())
	{
		...
		FPlatformProcess::LaunchURL(TEXT("https://s2w-dev-firebase.herokuapp.com/api/oauth2/authorize?response_type=code&scope=email&redirect_uri=http://localhost:8080/get&client_id=pmQegCYcs446TVEvQA2M"), NULL, NULL);

		/*UnrealHttp::api::HttpService* httpService = new UnrealHttp::api::HttpService();;
		UnrealHttp::api::HttpService::PoqGetAccessTokenRequest request;
		request.code = "awaefwaefwaefwe";
...
bool AHttpServer::RequestGET(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	{
		if(queryParam.Key.Equals(FString("code")))
		{
			code_ = queryParam.Value;
		}
			
	}

	if(!code_.Len())
		FPlatformProcess::LaunchURL(TEXT("https://s2w-dev-firebase.herokuapp.com/api/oauth2/authorize?response_type=code&scope=wallet&redirect_uri=http://localhost:8080/get&client_id=pmQegCYcs446TVEvQA2M"), NULL, NULL);
	else
	{
		UE_LOG(LogTemp, Log, TEXT("QueryParam = 'code' : '%s'"), *code_);
		UnrealHttp::api::HttpService* httpService = new UnrealHttp::api::HttpService();;
		UnrealHttp::api::HttpService::PoqGetAccessTokenRequest request;
		request.code = code_;
		httpService->getAccessToken(request);
